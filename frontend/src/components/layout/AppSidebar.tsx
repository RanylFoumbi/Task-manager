...
import { Home, Folder, Users, BarChart3, LayoutDashboard, Plus, BadgeInfo } from "lucide-react"
...
export default function AppSidebar({ isOpen }: AppSidebarProps) {
  ...
  return (
    ...
    >
      <div className={isOpen ? "flex items-center gap-3 p-4 border-b border-slate-200 h-12" : "flex flex-col items-center p-4 border-b border-slate-200 h-12"}>
          <div className="flex h-5 w-5 items-center justify-center rounded-lg bg-primary text-primary-foreground">
            <span className="text-xs font-bold">{getFirstLetterCapitalized(appName)}</span>
          </div>
          {isOpen && <span className="text-sm font-semibold text-slate-900">{appName}</span>}
      </div>
      ...
      {isOpen && (
        <div className="border-t border-slate-200 p-4">
          <p className="text-xs font-semibold text-slate-500 uppercase mb-3">Favoris</p>
